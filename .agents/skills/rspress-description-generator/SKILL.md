...
Before modifying more than one file, list the exact candidate paths and ask the user to confirm the batch scope.
After confirmation, process files by directory to maintain consistency. Do not delegate or parallelize the edits unless the user explicitly requests it.
