...
3. **Scaffold Rspress v2**
   ...
   - Before running a scaffold or dependency-install command, show the exact command and target directory and obtain user approval; these commands download and may execute third-party code.

     ```bash
     # interactive (recommended for first-time setup)
     pnpm create rspress@2
     yarn create rspress@2
     npm create rspress@2
     bun create rspress@2

     # or non-interactive, e.g. for CI/automation
     pnpm dlx create-rspress@2 my-docs --template basic-theme --tools rslint,prettier
     yarn dlx create-rspress@2 my-docs --template basic-theme --tools rslint,prettier
     npx -y create-rspress@2 my-docs --template basic-theme --tools rslint,prettier
     bun x create-rspress@2 my-docs --template basic-theme --tools rslint,prettier
     ```

   - After scaffolding, verify the generated docs `package.json` depends on Rspress v2 through `@rspress/core` with a 2.x range. Do not accept a generated `rspress` dependency as v2. If the dependency is missing, still v1, or uses the legacy package, stop and report the mismatch instead of retrying with another major version.
