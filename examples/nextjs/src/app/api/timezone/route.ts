...
/**
 * Get or create a cached DateTimeFormat instance for timezone label
 * @param tz - Timezone string
 * @returns Cached or new DateTimeFormat instance
 */
function getLabelFormatter(tz: string): Intl.DateTimeFormat {
  let formatter = labelFormatterCache.get(tz);
  ...
  if (!formatter) {
    ...
    labelFormatterCache.set(tz, formatter);
  }
...
function getTimezoneLabel(tz: string): string {
  try {
    const now = Date.now();
    const formatter = getLabelFormatter(tz);
