...
For the starred transitions, `Again`, `Hard`, or `Good` can lead to `Review` instead of `Learning` or `Relearning` when the duration calculated for that rating is zero or at least one day, or no step applies.
