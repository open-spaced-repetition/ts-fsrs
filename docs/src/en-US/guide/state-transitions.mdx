...
The arrow labels show ratings. An asterisk means the transition depends on whether a positive short-term step remains.

## What each state means

| State | Meaning | Default queue |
| --- | --- | --- |
| `New` | The card has not been reviewed yet. | `new` |
| `Learning` | The card is progressing through its initial short-term learning steps. | `learning` |
| `Review` | The model schedules the card with a long-term interval. | `review` |
...
| `Relearning` | Return to the first step | Repeat the current step | Next step or `Review` | `Review` |

A positive step shorter than one day keeps the card in `Learning` or `Relearning`. A step of one day or longer uses the exact due time but moves the card to `Review`. When short-term scheduling is disabled, the scheduler uses the model interval. It currently also falls back to the model interval when no positive step applies; this is a known issue, not the intended learning-step behavior.

:::warning Learning states require learning-step policy
`DefaultScheduler` registers `schedulerLearningStepsMiddleware` for you, but it only creates short-term transitions when `enableShortTerm` is `true`. When composing a scheduler with `defineScheduler`, add `.use(schedulerLearningStepsMiddleware)` and configure positive steps. If short-term scheduling is not enabled, or the middleware is not registered, reviews use the model's long-term interval and do not enter `Learning` or `Relearning`.
:::
...
Use `card.state` to explain the learning phase, and `card.scheduleStatus` to filter scheduling queues.
