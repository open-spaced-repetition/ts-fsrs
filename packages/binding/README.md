...
The third argument must be a numeric decay in the range of `[0.1, 1]`: FSRS-4 uses `1`, FSRS-4.5/5 uses `0.5`, and FSRS-6 uses `weights[20]`. Parameter arrays are not accepted. This estimator is not usable with FSRS-3 (exponential curve) or FSRS-7 (dual-trace curve).
