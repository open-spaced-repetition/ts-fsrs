...
export type FSRS4Config = FSRSModelConfig

export const FSRS4Model = (config: FSRS4Config): IFSRSModel<FSRS4Config> => {
  const bounds = FSRS4_MODEL_BOUNDS

  const modelConfig: FSRS4Config = Object.freeze(config)

  const algo = new FSRS4Algorithm(modelConfig.weights, FSRS4_MODEL_BOUNDS)
