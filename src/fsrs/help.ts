...
export function show_diff_message(due: Date, last_review: Date, unit?: boolean) {
  const diff = due.getTime() - last_review.getTime();

  if (diff < 1000 * 60 * 60) { // 小于1小时
    return unit ? Math.floor((diff) / (60 * 1000)) + 'min' : Math.floor((diff) / (60 * 1000));
  } else if (diff >= 1000 * 60 * 60 && diff < 1000 * 60 * 60 * 24) { // 大于1小时,小于1天
    return unit ? Math.floor((diff) / (60 * 60 * 1000)) + 'hour' : Math.floor((diff) / (60 * 60 * 1000));
  } else if (diff >= 1000 * 60 * 60 * 24 && diff < 1000 * 60 * 60 * 24 * 31) { // 大于1天,小于31天
    return unit ? Math.floor((diff) / (24 * 60 * 60 * 1000)) + 'day' : Math.floor((diff) / (24 * 60 * 60 * 1000 * 1000));
